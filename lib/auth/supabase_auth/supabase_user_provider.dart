...
class SnapchartSupabaseUser extends BaseAuthUser {
  SnapchartSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> snapchartSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SnapchartSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
